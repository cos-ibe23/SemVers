import { describe, it, expect, beforeEach, afterAll } from 'vitest';
import { getTestDb, cleanTestDb, closeTestDb } from '@test/helpers';
import { createIntegrationTestApp, signupAndLogin } from '@test/helpers/integration';
import { createUserFactory, createClientFactory, UserFactory, ClientFactory } from '@test/factories';
import { shipperClients } from '../../../db/schema';
import { eq } from 'drizzle-orm';

describe('Shipper Clients Routes (Integration)', () => {
    const db = getTestDb();
    const app = createIntegrationTestApp();
    let userFactory: UserFactory;
    let clientFactory: ClientFactory;
    
    let shipperAuth: { headers: Record<string, string>, user: any };

    beforeEach(async () => {
        await cleanTestDb();
        userFactory = createUserFactory(db);
        clientFactory = createClientFactory(db);
        UserFactory.resetCounter();
        ClientFactory.resetCounter();
        
        // Setup authenticated shipper
        shipperAuth = await signupAndLogin('shipper@example.com', 'Test Shipper');
    });

    afterAll(async () => {
        await closeTestDb();
    });

    describe('GET /clients', () => {
        it('should return 401 without authentication', async () => {
             // Create new app instance without auth headers or just request without them
            const response = await app.request('/v1/clients', {
                method: 'GET',
            });
            expect(response.status).toBe(401);
            const body = await response.json();
            expect(body).toHaveProperty('error');
        });

        it('should return empty list when shipper has no clients', async () => {
            const response = await app.request('/v1/clients', {
                method: 'GET',
                headers: shipperAuth.headers,
            });

            expect(response.status).toBe(200);
            const body = await response.json();
            expect(body.data).toEqual([]);
            expect(body.total).toBe(0);
        });

        it('should return clients for shipper', async () => {
            await clientFactory.createForShipper(shipperAuth.user.id, { name: 'Client A' });
            await clientFactory.createForShipper(shipperAuth.user.id, { name: 'Client B' });

            const response = await app.request('/v1/clients', {
                method: 'GET',
                headers: shipperAuth.headers,
            });

            expect(response.status).toBe(200);
            const body = await response.json();
            expect(body.data).toHaveLength(2);
            expect(body.total).toBe(2);
        });

        it('should not return other shippers clients', async () => {
            // Create another shipper (we don't need to login as them, just insert db record)
            const otherShipper = await userFactory.createShipper({ email: 'other@example.com' });
            
            await clientFactory.createForShipper(shipperAuth.user.id, { name: 'Shipper 1 Client' });
            await clientFactory.createForShipper(otherShipper.id, { name: 'Shipper 2 Client' });

            const response = await app.request('/v1/clients', {
                method: 'GET',
                headers: shipperAuth.headers,
            });

            expect(response.status).toBe(200);
            const body = await response.json();
            expect(body.data).toHaveLength(1);
            expect(body.data[0].client.name).toBe('Shipper 1 Client');
        });
    });

    describe('POST /clients', () => {
        it('should validate request body - requires clientUserId or name+email', async () => {
            const response = await app.request('/v1/clients', {
                method: 'POST',
                headers: { 
                    'Content-Type': 'application/json',
                    ...shipperAuth.headers
                },
                body: JSON.stringify({}),
            });

            expect(response.status).toBe(422);
            const body = await response.json();
            expect(body).toHaveProperty('error');
        });

        it('should add existing client user to shipper', async () => {
            const clientUser = await clientFactory.createUser({ name: 'Existing Client' });

            const response = await app.request('/v1/clients', {
                method: 'POST',
                headers: { 
                    'Content-Type': 'application/json',
                    ...shipperAuth.headers
                },
                body: JSON.stringify({
                    clientUserId: clientUser.id,
                    nickname: 'My Nickname',
                }),
            });

            expect(response.status).toBe(201);
            const body = await response.json();
            expect(body.clientId).toBe(clientUser.id);
            expect(body.nickname).toBe('My Nickname');
            expect(body.client.name).toBe('Existing Client');
        });

        it('should create new client user and add to shipper', async () => {
            const response = await app.request('/v1/clients', {
                method: 'POST',
                headers: { 
                    'Content-Type': 'application/json',
                    ...shipperAuth.headers
                },
                body: JSON.stringify({
                    name: 'New Client',
                    email: 'newclient@example.com',
                    nickname: 'NC',
                }),
            });

            expect(response.status).toBe(201);
            const body = await response.json();
            expect(body.client.name).toBe('New Client');
            expect(body.client.email).toBe('newclient@example.com');
            expect(body.nickname).toBe('NC');
        });

        it('should return 400 if client already added', async () => {
            const { user: clientUser } = await clientFactory.createForShipper(shipperAuth.user.id);

            const response = await app.request('/v1/clients', {
                method: 'POST',
                headers: { 
                    'Content-Type': 'application/json',
                    ...shipperAuth.headers
                },
                body: JSON.stringify({
                    clientUserId: clientUser.id,
                }),
            });

            expect(response.status).toBe(400);
            const body = await response.json();
            expect(body.error).toContain('already added');
        });
    });

    describe('GET /clients/:clientId', () => {
        it('should return 404 for non-existent client', async () => {
            const response = await app.request('/v1/clients/non-existent-id', {
                method: 'GET',
                headers: shipperAuth.headers,
            });

            expect(response.status).toBe(404);
        });

        it('should return client details', async () => {
            const { user: clientUser } = await clientFactory.createForShipper(shipperAuth.user.id, {
                name: 'My Client',
                nickname: 'MC',
            });

            const response = await app.request(`/v1/clients/${clientUser.id}`, {
                method: 'GET',
                headers: shipperAuth.headers,
            });

            expect(response.status).toBe(200);
            const body = await response.json();
            expect(body.clientId).toBe(clientUser.id);
            expect(body.nickname).toBe('MC');
            expect(body.client.name).toBe('My Client');
        });
    });

    describe('PATCH /clients/:clientId', () => {
        it('should update client nickname and phone', async () => {
            const { user: clientUser } = await clientFactory.createForShipper(shipperAuth.user.id, {
                nickname: 'Old Nickname',
            });

            const response = await app.request(`/v1/clients/${clientUser.id}`, {
                method: 'PATCH',
                headers: { 
                    'Content-Type': 'application/json',
                    ...shipperAuth.headers
                },
                body: JSON.stringify({
                    nickname: 'New Nickname',
                    phone: '+1234567890',
                }),
            });

            expect(response.status).toBe(200);
            const body = await response.json();
            expect(body.nickname).toBe('New Nickname');
            expect(body.phone).toBe('+1234567890');
        });
    });

    describe('DELETE /clients/:clientId', () => {
        it('should soft delete client relationship', async () => {
            const { user: clientUser } = await clientFactory.createForShipper(shipperAuth.user.id);

            // Delete the client
            const deleteResponse = await app.request(`/v1/clients/${clientUser.id}`, {
                method: 'DELETE',
                headers: shipperAuth.headers,
            });
            expect(deleteResponse.status).toBe(200);

            // Verify client no longer appears in list
            const listResponse = await app.request('/v1/clients', {
                method: 'GET',
                headers: shipperAuth.headers,
            });
            const body = await listResponse.json();
            expect(body.data).toHaveLength(0);
        });
    });
});
